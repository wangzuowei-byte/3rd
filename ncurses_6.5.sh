...
PTHREAD_NAME=ncurses_source
TAR_NAME=ncurses-6.5.tar.gz
...
function aarch64_qnx710_build()
{
   ...
   ../configure                                 \
    ...
    CC="${CC} --sysroot=${SYSROOT}" \
    CXX="${CXX} --sysroot=${SYSROOT}" \
    CPPFLAGS="-I${SDKTARGETSYSROOT}/usr/include -DNO_TERMIO_ERROR -D__TERMIO_H_INCLUDED -D__TERMIOS_H_INCLUDED -DNCC=18" \
    CFLAGS="-I${SDKTARGETSYSROOT}/usr/include -DNO_TERMIO_ERROR -D__TERMIO_H_INCLUDED -D__TERMIOS_H_INCLUDED -DNCC=18" \
    LDFLAGS="-L${SDKTARGETSYSROOT}/usr/lib" \
    --prefix=${PRJ_INSTALL_PATH}
#   --disable-stripping
